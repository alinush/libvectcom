...
#include <vectcom/RootsOfUnityEval.h>
...
using libvectcom::Fr;
using libvectcom::G1;
using libvectcom::random_group_elems;
using libvectcom::random_field_elems;
...
void benchAllRootsOfUnity(size_t n, size_t r) {
    ...
    for(size_t id = 0; id < n; id++) {
        ...
        for(size_t i = 0; i < r; i++) {
            ...
            tn.startLap();
            libvectcom::multiExp<G1>(bases, exp);
            tn.endLap();
        }
...
int main(int argc, char *argv[]) {
    ...
    //exp = libvectcom::get_all_roots_of_unity(Utils::log2ceil(n));
    //exp.resize(n);
