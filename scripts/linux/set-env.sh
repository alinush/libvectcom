...
if [ $# -lt 1 ]; then
    ...
    echo
    if [ -n "$VECTCOM_HAS_ENV_SET" ]; then
        echo "Currently-set environment"
        echo "========================="
        echo "Build directory: $VECTCOM_BUILD_DIR"
        echo "CMake args: $VECTCOM_CMAKE_ARGS"
    fi
else
    ...
    branch=${branch/\* /}
    builddir_base=~/builds/vectcom/$branch
    case "$buildtype" in
    trace)
    ...
    *)
    ...
    if [ $invalid_buildtype -eq 0 ]; then
        ...
        export PATH="$builddir/libvectcom/bin:$PATH"
        export PATH="$builddir/libvectcom/bin/test:$PATH"
        export PATH="$builddir/libvectcom/bin/bench:$PATH"
        export PATH="$builddir/libvectcom/bin/app:$PATH"
        export PATH="$builddir/libvectcom/bin/examples:$PATH"

        echo "Build directory: $builddir"
        echo "CMake args: $cmake_args"
        echo "PATH envvar: $PATH" 

        export VECTCOM_BUILD_DIR=$builddir
        export VECTCOM_CMAKE_ARGS=$cmake_args
        export VECTCOM_HAS_ENV_SET=1
        export VECTCOM_SOURCE_DIR=$sourcedir
    else
